...
func generateJSONTag(name string) reflect.StructTag {
	...
	case 0:
		return reflect.StructTag(`json:"-"`) // always omitted
	case 1:
		opt = "" // empty opt
	case 2:
		opt = "omitempty"
	// case 3: opt = "string"
	default:
		return reflect.StructTag("") // empty tag
	}
...
func Map2StructType(m map[string]interface{}, maxDepth int) reflect.Type {
	...
	for k, v := range m {
		...
		if ft == nil {
			...
			switch ft.Kind() {
			case reflect.Map:
				ft = Map2StructType(v.(map[string]interface{}), maxDepth-1)
			case reflect.Float64:
				ft = generateNumberType()
			}
		}
